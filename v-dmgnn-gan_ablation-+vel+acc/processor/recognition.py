...
class REC_Processor(Processor):
    ...
    def train(self, masking_type="lower-body"):
        ...
        if  self.meta_info['iter'] % 2 == 0:
            with torch.no_grad():
                mean_p, var_p, mean_v, var_v, mean_a, var_a, gan_decoder_inputs, \
    ...
    def compute_z(self, mean, log_var):
        var = torch.exp(0.5 * log_var) # TBD
    ...
    def train_decoder(
        ...
        gan_decoder_inputs_previous2):
        with torch.no_grad():
            z_p = self.compute_z(mean_p, var_p)
            z_v = self.compute_z(mean_v, var_v)
            z_a = self.compute_z(mean_a, var_a)
            z = torch.cat((z_p, z_v, z_a), dim=1).permute(0,2,1).permute(0,2,1)
    ...
    def train_generator(self, mode='generator', masking_type="lower-body"):
        ...
        if mode =='generator':
            loss = self.vae_loss_function(outputs, targets, mean_p, log_var_p)
        ...
        return mean_p, log_var_p, mean_v, log_var_v, mean_a, log_var_a, \
            gan_decoder_inputs, gan_targets, gan_decoder_inputs_previous, \
            gan_decoder_inputs_previous2, gan_disc_encoder_inputs
    ...
    def test(
        ...
        for action_num, action in enumerate(self.actions):
            ...
            with torch.no_grad():
                outputs, _, _, _, _, _, _ = self.model(
                    encoder_inputs_p,
                    encoder_inputs_v,
                    encoder_inputs_a,
                    decoder_inputs,
