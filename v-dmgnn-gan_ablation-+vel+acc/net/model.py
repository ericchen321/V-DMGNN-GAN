...
class Model(nn.Module):
    ...
    def forward(self, enc_p, enc_v, enc_a, dec_curr, dec_prev, dec_prev2,
        ...
        mean_p, log_var_p = self.encoder_pos(enc_p, relrec_s1, relsend_s1, relrec_s2, relsend_s2, relrec_s3, relsend_s3, lamda_p)
        mean_v, log_var_v = self.encoder_vel(enc_v, relrec_s1, relsend_s1, relrec_s2, relsend_s2, relrec_s3, relsend_s3, lamda_p)
        mean_a, log_var_a = self.encoder_vel(enc_a, relrec_s1, relsend_s1, relrec_s2, relsend_s2, relrec_s3, relsend_s3, lamda_p)
        latent_var_p = self.reparameterization(mean_p, torch.exp(0.5 * log_var_p))
        latent_var_v = self.reparameterization(mean_v, torch.exp(0.5 * log_var_v))
        latent_var_a = self.reparameterization(mean_a, torch.exp(0.5 * log_var_a))

        #hidden = self.linear(latent_var.permute(0, 2, 1)).permute(0, 2, 1)
        hidden = self.linear(torch.cat(
            (latent_var_p, latent_var_v, latent_var_a), dim=1).permute(0,2,1)).permute(0,2,1)
        hidden = self.relu(hidden)

        preds = self.decoder(dec_curr, dec_prev, dec_prev2, hidden, t)
        return preds, mean_p, log_var_p, mean_v, log_var_v, mean_a, log_var_a
...
class Decoder(nn.Module):
    ...
    def forward(self, inputs, inputs_previous, inputs_previous2, hidden, t):      # inputs:[64, 1, 63];  hidden:[64, 256, 21]
        ...
        for step in range(0, t):
            ...
            '''
            # n = torch.randn(ins_p.size()).cuda()*0.0005
            ins = torch.cat((ins_p, ins_v, ins_a), dim=-1)
            '''
...
class Discriminatorv3(nn.Module):
    ...
    def forward(self, inputs):      # inputs:[64, 1, 63];  hidden:[64, 256, 21]
        # pred_all = []
        # res_all = []
        hiddens= []
        ...
        for step in range(0, t):
            ...
            if step < 1:
                ...
                ins_p = inputs[:, step, :, :] # pred_all[step-1]                                           # ins_p: [64, 21, 3]
                ins_v = (inputs-inputs_previous)[:, 0, :, :]                                   # ins_v: [64, 21, 3]
                ins_a = ins_p-inputs[:, 0, :, :]-ins_v # ins_v-(inputs-inputs_previous)[:, 0, :, :]
                ins_v_dec = inputs[:, step, :, :]-inputs[:, 0, :, :]
            elif step==2:
                ins_p = inputs[:, step, :, :]# pred_all[step-1]
                ins_v = inputs[:, 1, :, :]-inputs[:, 0, :, :] #pred_all[step-2]-inputs[:, 0, :, :]
                ins_a = ins_p-inputs[:, 1, :, :]-ins_v # ins_v-(pred_all[step-2]-inputs[:, 0, :, :])
                ins_v_dec = inputs[:, step, :, :]-inputs[:, 1, :, :]# pred_all[step-1]-pred_all[step-2]
            else:
                ins_p = inputs[:, step, :, :] # pred_all[step-1]
                ins_v = inputs[:, step-1, :, :]-inputs[:, step-2, :, :]
                ins_a = ins_p-inputs[:, step-1, :, :]-ins_v # ins_v-(pred_all[step-2]-pred_all[step-3])
                ins_v_dec = inputs[:, step, :, :]-inputs[:, step-1, :, :]
            ...
            (Pdb) print(in_channels)
            3
            (Pdb) print(out_channels)
            32
            (Pdb) print(k_num)
            2
            (Pdb) t_kernel_size
        ...
        return output#hiddens[-1] # preds.transpose(1, 2).contiguous()      # [64, 21, t, 3]
